<template>
	<v-card height="calc(100vh - 70px)" style="border-radius:0px;" v-if="modelComputed">
			...
			<v-tab-item>
				<v-container>
					<v-layout>
						<blockquote
							class="blockquote"
							style="height: calc(100vh - 150px); width:100%; overflow-y: scroll;"
						>
							<span class="display-2">{{modelTitle}} {{modelFields.id.value}}</span>
							<br />
							<br />
							...
							<div style="margin-top:20px;" v-for="(v,k) in modelChildren">
								<span class="display-1">{{v.title}}</span>
								<br />
								<ul style="list-style-type: none; padding-left: 0px;">
...
<script>
	...
	export default {
		...
		computed: {
			...
			modelTitle() {
				return this.$store.state.model.title;
			},
			modelFields() {
				return this.$store.state.model.form_fields.fields;
			},
			modelChildren() {
				return this.$store.state.model.form_fields.children;
			},
			modelRelations() {
				return this.$store.state.model.form_fields.relations;
			},
			schoolingsExist() {
				return "schoolings" in this.$store.state.model.form_fields ? true : false;
			},
			modelSchoolings() {
				return this.$store.state.model.form_fields.schoolings;
			},
			isEditable() {
