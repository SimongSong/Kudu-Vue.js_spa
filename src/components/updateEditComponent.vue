					...
					<div v-for="c in childrenDifferences">
						<div class="headline font-weight-light" v-if="c.length !== 0">
							<div v-if="c[1].length !== 0">
								For
								<span :style="{'color':colour}">{{c[0]}}</span>:
								<br />
								<div v-for="d in c[1]">
									&emsp;Field
									<span :style="{'color':colour}">{{titleEdit(d[0])}}</span> has been updated from
									<span :style="{'color':colour}">{{printNone(d[1])}}</span> to
									<span :style="{'color':colour}">{{printNone(d[2])}}</span>.
								</div>
								<v-divider></v-divider>
							</div>
						</div>
		...
		computed: {
			...
			childrenDifferences() {
				...
				Object.keys(this.children).forEach(key => {
					diffObject.push([
						this.children[key].title,
						this.computeDifferences(
							this.children[key].fields,
							this.structure.children[key].fields
						)
					]);
				});
				return diffObject;
			...
			relationsDifferences() {
				...
				Object.keys(this.relations).forEach(key => {
					if (
						this.structure.relations[key].selected.length == 0 &&
						this.relations[key].selected.length == 0
					)
						return;
					var title = this.getTitle(
						this.relations[key].model[0],
						this.relations[key].model[1]
					);
					if (
						!this.arraysEqual(
							this.relations[key].selected,
							this.structure.relations[key].selected
						)
					) {
						diffObject.push([
							title,
			...
			isDifferent() {
				if (
					this.relationsDifferences.length === 0 &&
		...
		methods: {
			...
			computeDifferences(oldObject, newObject) {
				var differenceList = Object.keys(oldObject).reduce((acc, curr) => {
					if (oldObject[curr].value !== newObject[curr].value) {
						acc.push([curr, oldObject[curr].value, newObject[curr].value]);
					}
					return acc;
				}, []);
